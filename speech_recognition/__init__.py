...
class Recognizer(AudioSource):
    ...
    def recognize_sphinx(
        ...
        assert (
            ...
            or (isinstance(language, tuple) and len(language) == 3)
    ...
    def recognize_azure(
        ...
        access_token, expire_time = (
            getattr(self, "azure_cached_access_token", None),
    ...
    def recognize_bing(self, audio_data, key, language="en-US", show_all=False):
        ...
        access_token, expire_time = (
            getattr(self, "bing_cached_access_token", None),
    ...
    def recognize_houndify(self, audio_data, client_id, client_key, show_all=False):
        ...
        wav_data = audio_data.get_wav_data(
            convert_rate=None
            if audio_data.sample_rate in [8000, 16000]
            else 16000,  # audio samples must be 8 kHz or 16 kHz
            convert_width=2,  # audio samples should be 16-bit
        )
        ...
        if "Disambiguation" not in result or result["Disambiguation"] is None:
            raise UnknownValueError()
        return result["Disambiguation"]["ChoiceData"][0]["Transcription"], result[
            "Disambiguation"
        ]["ChoiceData"][0]["ConfidenceScore"]
    ...
    def recognize_amazon(
        ...
        if check_existing:
            ...
            try:
                ...
                if (
                    exc.response["Error"]["Code"] == "BadRequestException"
                    and "The requested job couldn't be found" in str(exc)
                ):
    ...
    def recognize_ibm(self, audio_data, key, language="en-US", show_all=False):
        ...
        flac_data = audio_data.get_flac_data(
            ...
            else 16000,  # audio samples should be at least 16 kHz
            convert_width=None
            if audio_data.sample_width >= 2
            else 2,  # audio samples should be at least 16-bit
        )
    ...
    def recognize_distilwhisper(
        ...
        **transcribe_options,
        ...
        result = whisper(
            audio_array, chunk_length_s=50, stride_length_s=10, batch_size=8
        )
        # print(result)
        if show_dict:
            return result
